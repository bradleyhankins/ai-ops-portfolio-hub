...
CLIENTOPS_REPO_URL = "https://github.com/bradleyhankins/clientops-intake-ai"
CLIENTOPS_LIVE_URL = "https://clientops-intake-ai.streamlit.app/"
...
TOOL_SELECTOR = {
    ...
    "I need to diagnose my business workflows": "ClientOps Intake AI",
...
def project_card(name: str, project: dict) -> None:
    ...
    status = project.get("status", "Live")
    status_class = "status-live" if status == "Live" else "status-next"
    st.markdown(f"""
        <div class="project-card">
            <div class="project-topline"><h3 class="project-title">{name}</h3><span class="status-pill {status_class}">{status}</span></div>
    ...
    with col1:
        link_button("Live Demo" if project.get("live") else "Live Demo Coming Soon", project.get("live"))
    with col2:
        link_button("GitHub" if project.get("github") else "Repo Coming Soon", project.get("github"))
...
with c1:
    stat_card("Toolkit Projects", "5")
with c2:
    stat_card("Live Apps", "4")
with c3:
...
r1, r2 = st.columns(2)
with r1:
    link_button(f"Launch {recommended_name}" if recommended.get("live") else "Live Demo Coming Soon", recommended.get("live"))
with r2:
    link_button(f"View {recommended_name} on GitHub" if recommended.get("github") else "Repository Coming Soon", recommended.get("github"))
...
with u1:
    ...
    usecase_card("Need a business diagnostic?", "ClientOps Intake AI", "Coming next: diagnose workflow bottlenecks, maturity level, and the best automation opportunity.")
with u2:
...
with b1:
    card("1. Find repeatable pain", "Each app starts with a recurring business workflow problem: reporting, follow-up, applicant review, documentation, or client diagnostics.")
with b2:
    card("2. Map the workflow", "The process is converted into inputs, decision rules, outputs, and manager-ready documentation.")
with b3:
    card("3. Ship a working tool", "Each project is deployed as a Streamlit app with sample data, GitHub documentation, and downloadable outputs.")
...
with sp1:
    link_button(f"Launch {selected_project}" if spotlight.get("live") else "Live Demo Coming Soon", spotlight.get("live"))
with sp2:
    link_button(f"View {selected_project} on GitHub" if spotlight.get("github") else "Repository Coming Soon", spotlight.get("github"))

st.markdown('<div class="section-title">Portfolio story</div>', unsafe_allow_html=True)
st.markdown('<div class="section-lede">The toolkit was built around connected business problems: managers need performance visibility, sales teams need follow-up discipline, growing teams need structured applicant review, organizations need clearer process documentation, and small businesses need a better way to diagnose workflow bottlenecks.</div>', unsafe_allow_html=True)
...
p5, _ = st.columns(2)
with p5:
    project_card("ClientOps Intake AI", PROJECTS["ClientOps Intake AI"])

st.markdown('<div class="section-title">Toolkit roadmap</div>', unsafe_allow_html=True)
r1, r2 = st.columns(2)
with r1:
    st.markdown('<div class="roadmap-card"><h3>Near-Term Upgrades</h3><ul><li>Build ClientOps Intake AI</li><li>Add ClientOps live demo and GitHub repo</li><li>Add richer export formats</li><li>Add stronger role-specific templates</li><li>Explore PDF export options</li></ul></div>', unsafe_allow_html=True)
with r2:
...
st.markdown('<div class="final-cta"><h2>Practical AI tools built to solve real operational problems.</h2><p>Operations visibility. Sales execution. Applicant review organization. Process documentation. Client workflow diagnostics.</p></div>', unsafe_allow_html=True)
